...
def send_rules(update, chat_id, from_pm=False):	def send_rules(update, chat_id, from_pm=False):
    bot = dispatcher.bot	    bot = dispatcher.bot
    user = update.effective_user  # type: Optional[User]	    user = update.effective_user  # type: Optional[User]
    reply_msg = update.message.reply_to_message
    try:	    try:
        chat = bot.get_chat(chat_id)	        chat = bot.get_chat(chat_id)
    except BadRequest as excp:	    except BadRequest as excp:
@@ -53,6 +54,19 @@ def send_rules(update, chat_id, from_pm=False):
